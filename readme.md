...
* `今日人设`：每天为同一用户生成固定的称号、签运、幸运色、幸运食物与宜忌
* `骰子 2d6`：积分骰子，费用为 `3 × 骰子数量 + 骰子面数`，单局费用最多 1000 积分
* 骰子输/平/赢概率为 40%/40%/20%；输局无返奖、平局全额返还、赢局依据点数返还 2～4 倍费用，最多返奖 4000 积分
* `猜拳 石头 100`：和贝拉猜拳，最后的积分参数可省略；获胜得到等额积分、失败扣除、平局退回
* 可通过 `猜拳单次押注积分上限` 控制风险，设置为 0 时仍可进行无押注猜拳
...
* 骰子升级为积分玩法，加入费用、40%/40%/20% 输平赢权重及最高 4000 积分返奖
